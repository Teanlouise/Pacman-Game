...
/**
 * Hunters are entities which are controlled by the player to clear the board and win the game.  Hunters in this
 * version have special abilities like "phasing through ghost's" and "run's two blocks at a time" and so on.
...
public abstract class Hunter extends Entity {
     ...
     * If the duration for the special is greater than zero then use the hunter's special and set the special's
     * duration to the given duration. If the duration for the special is zero or lower than do not change the
     * special duration and do not use up the hunter's special.
    ...
    /**
     * Checks to see if the hunter is at the same position of the ghost. If the ghost and hunter do have the same
     * position then if the ghost is Phase.FRIGHTENED the ghost is killed Ghost.kill() otherwise the ghost kills
     * the hunter. If the ghost and hunter are not at the same position then do nothing.
